...
class GaugeSeekBar : View {
    ...
    private var thumbColor: Int = ContextCompat.getColor(context, R.color.md_amber_900)
    ...
    private fun init(centerX: Float, centerY: Float) {
        ...
        if (showThumb) {
            val thumbDrawable =
                if (thumbDrawableId != 0) ContextCompat.getDrawable(context, thumbDrawableId)!! else ThumbDrawable(
                    thumbColor
                )
